...
export async function attemptSteal(stealerId, targetId, groupId, pushName) {
    ...
    const success = Math.random() < Math.min(0.9, (STEAL_SUCCESS_CHANCE + weaponBonus) * stealMultiplier)
    let movedAmount = 0
...
export async function resolveHeist(groupId, targetId, initiatorId, crewIds, weaponBonus = 0) {
    ...
    const success = Math.random() < Math.min(0.9, (ROB_SUCCESS_CHANCE + weaponBonus) * robMultiplier)
