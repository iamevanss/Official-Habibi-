...
// Panel-only — deliberately no in-chat command for this, so a rename always
// goes through you rather than being self-serve for gang leaders.
export async function adminRenameGang(gangId, newName) {
    const trimmed = String(newName || '').trim()
    if (!trimmed || trimmed.length > 30) return { error: 'Gang names are 1-30 characters.' }

    const { error } = await supabase.from('gangs').update({ name: trimmed }).eq('id', gangId)
    if (error) {
        if (String(error.message).includes('duplicate')) return { error: `*${trimmed}* is already taken by another gang.` }
        console.error('Error renaming gang:', error.message)
        return { error: 'Something broke renaming that gang.' }
    }
...
// Win/loss record + recent history for the caller's gang — surrendered
// counts as a loss for the surrendering side. Pending/active wars are
// excluded; this is history, not status (use getGangWar for the current one).
export async function getGangWarStats(memberId) {
    const gang = await getGangByMember(memberId)
    if (!gang) return { error: "You're not in a gang." }
    ...
    const { data, error } = await supabase
        .from('gang_wars')
        .select('*')
        .or(`challenger_gang_id.eq.${gang.id},rival_gang_id.eq.${gang.id}`)
        .in('status', ['resolved', 'surrendered'])
        .order('resolved_at', { ascending: false })
        .limit(10)

    if (error) {
        console.error('Error fetching gang war history:', error.message)
        return { gang, wins: 0, losses: 0, ties: 0, totalWon: 0, recent: [] }
    }
    if (!data || !data.length) return { gang, wins: 0, losses: 0, ties: 0, totalWon: 0, recent: [] }

    const opponentIds = [...new Set(data.map((w) => (w.challenger_gang_id === gang.id ? w.rival_gang_id : w.challenger_gang_id)))]
    const { data: opponentGangs } = await supabase.from('gangs').select('id, name').in('id', opponentIds)
    const nameById = new Map((opponentGangs || []).map((g) => [g.id, g.name]))

    let wins = 0, losses = 0, ties = 0, totalWon = 0
    const recent = data.map((war) => {
        const opponentId = war.challenger_gang_id === gang.id ? war.rival_gang_id : war.challenger_gang_id
        const pot = Number(war.wager) * 2
        let result
        if (war.winner_gang_id === gang.id) {
            result = 'won'
            wins++
            totalWon += pot
        } else if (war.winner_gang_id) {
            result = 'lost'
            losses++
        } else {
            result = 'tie'
        ...
        return {
            opponentName: nameById.get(opponentId) || 'a gang that no longer exists',
            result,
            pot,
            resolvedAt: war.resolved_at,
            surrendered: war.status === 'surrendered'
        }
    })

    return { gang, wins, losses, ties, totalWon, recent }
}
...
const EVENT_TYPES = {
    ...
    lucky_hour: { label: 'Lucky Hour', emoji: '🍀', defaultMultiplier: 0.15, describe: (m) => `.steal, .rob, and .strip odds are all boosted +${Math.round(m * 100)}% for a limited time!` },
    jackpot_flip: { label: 'Jackpot Flip', emoji: '💰', defaultMultiplier: 3, describe: (m) => `.flip wins pay out ${m}x, with a 15% chance of an extra JACKPOT 2x on top!` },
    crime_spree: { label: 'Crime Spree', emoji: '🚨', defaultMultiplier: 2, describe: (m) => `Your .steal attempt limit is ${m}x higher for a limited time!` },
    free_revive: { label: 'Free Revive Hour', emoji: '❤️‍🩹', defaultMultiplier: 0, describe: (m) => m === 0 ? `.revive is completely FREE for a limited time!` : `.revive costs ${Math.round((1 - m) * 100)}% less for a limited time!` }
}

const JACKPOT_HIT_CHANCE = 0.15
...
// Sibling to the above, for event types whose "multiplier" field is actually
// a flat additive bonus (lucky_hour's 0.15 means "+15 percentage points",
// not "times 0.15") — otherwise 0 (safe no-op for addition).
export async function getFlatEventBonusFor(type) {
    const active = await getActiveEvent()
    return active && active.type === type ? active.multiplier : 0
}
...
export async function attemptSteal(stealerId, targetId, groupId, pushName) {
    ...
    // crime_spree's multiplier field is a cap multiplier (default 2 = double
    // the normal attempt limit), not an odds bonus — checked here since it
    // needs to run before the odds calc, not alongside it.
    const spreeMultiplier = await getEventMultiplierFor('crime_spree')
    const effectiveMaxAttempts = Math.round(STEAL_MAX_ATTEMPTS * spreeMultiplier)
    ...
    const luckyBonus = await getFlatEventBonusFor('lucky_hour')
    const success = Math.random() < Math.min(0.9, (STEAL_SUCCESS_CHANCE + weaponBonus + warContext.bonus + luckyBonus) * stealMultiplier)
    let movedAmount = 0
...
export async function resolveHeist(groupId, targetId, initiatorId, crewIds, weaponBonus = 0) {
    ...
    const luckyBonus = await getFlatEventBonusFor('lucky_hour')
    const success = Math.random() < Math.min(0.9, (ROB_SUCCESS_CHANCE + weaponBonus + warContext.bonus + luckyBonus) * robMultiplier)
...
export async function coinflip(memberId, amount, groupId, pushName) {
    ...
    // double_flip and jackpot_flip are mutually exclusive (only one event can
    // ever be active at a time), so checking both and taking whichever is
    // actually active is safe — never double-applies.
    const flipMultiplier = won ? await getEventMultiplierFor('double_flip') : 1
    const jackpotMultiplier = won ? await getEventMultiplierFor('jackpot_flip') : 1
    const isJackpotEvent = jackpotMultiplier !== 1
    const jackpotHit = isJackpotEvent && Math.random() < JACKPOT_HIT_CHANCE

    let winnings = 0
    if (won) {
        winnings = Math.round(amount * (isJackpotEvent ? jackpotMultiplier : flipMultiplier))
        if (jackpotHit) winnings *= 2
    }
    ...
    return { won, amount, winnings, jackpotHit, newAchievement }
}
...
export async function attemptStrip(actorId, targetId, groupId, pushName) {
    ...
    const luckyBonus = await getFlatEventBonusFor('lucky_hour')
    const success = Math.random() < Math.min(0.95, STRIP_SUCCESS_CHANCE + luckyBonus)
...
export async function reviveUser(actorId, targetId, pushName) {
    ...
    const discount = await getEventMultiplierFor('free_revive')
    const effectiveCost = Math.round(REVIVE_COST * discount)

    const actor = await getOrCreateUser(actorId, pushName)
    if (Number(actor.balance || 0) < effectiveCost) return { error: `You need ${effectiveCost.toLocaleString()} habz for that.` }

    if (effectiveCost > 0) {
        await updateUser(actorId, { balance: Number(actor.balance || 0) - effectiveCost })
    }
