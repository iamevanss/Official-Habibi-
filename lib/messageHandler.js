...
export async function handleIncomingMessage(sock, m) {
    try {
            ...
            switch (command) {
                ...
                case 'help': {
                    await reply(
                        ...
                        `• *.gang* : Start, join, or manage a gang (up to 10 members) — try \`.gang\` alone for the full subcommand list`,
                        [sender]
                    )
                    break
                }
                ...
                case 'revive': {
                    ...
                    await reply(
                        result.cost > 0
                            ? `⚡ ${senderTag} revived ${targetDisplayName} for ${formatHabz(result.cost)}. Fresh start, clean slate.`
                            : `⚡ ${senderTag} revived ${targetDisplayName} for free! Fresh start, clean slate.`,
                        [sender, mentionedJid]
                    )
                    break
                }
                ...
                case 'coinflip': {
                    ...
                    await reply(
                        result.won
                            ? result.jackpotHit
                                ? `🪙💰 *JACKPOT!* ${senderTag} hit the bonus multiplier! +${formatHabz(result.winnings)}`
                ...
                case 'gang': {
                    ...
                    if (sub === 'create') {
                        ...
                        if (!name) {
                            await reply('Usage: `.gang create <name>`')
                            break
                        }
                        const result = await createGang(sender, name, pushName)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(
                            `🔫 *Gang created: ${result.gang.name}*\n\n` +
                            `You're the Leader. Here's everything:\n\n` +
                            `*Members*\n` +
                            `• \`.gang invite @user\` — invite someone (max 10 members total)\n` +
                            `• \`.gang kick @user\` — remove someone\n` +
                            `• \`.gang promote @user\` / \`.gang demote @user\` — Officers can invite & kick too, only you can touch the vault or disband\n\n` +
                            `*Vault*\n` +
                            `• \`.gang deposit <amount>\` — anyone can add\n` +
                            `• \`.gang withdraw <amount>\` — leader only\n\n` +
                            `*War*\n` +
                            `• \`.gang war <gang name> <amount>\` — challenge a rival, staking habz from your vault\n` +
                            `• Stealing from an enemy gang during a war has boosted odds\n` +
                            `• Winner takes both pots after 24h — or \`.gang surrender\` to cut losses early\n\n` +
                            `*Other*\n` +
                            `• \`.gang info\` — see your gang's roster and vault\n` +
                            `• \`.gang transfer @user\` — hand off leadership\n` +
                            `• \`.gang disband\` — vault splits evenly among members\n\n` +
                            `Build it up, Leader.`,
                            [sender]
                        )
                        break
                    ...
                    if (sub === 'invite') {
                        if (!mentionedJid) {
                            await reply('Tag or reply to whoever you want to invite.')
                            break
                        }
                        const result = await inviteToGang(sender, mentionedJid)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(`📨 ${targetDisplayName} has been invited to *${result.gang.name}*. \`.gang accept\` or \`.gang reject\` to respond.`, [sender, mentionedJid])
                        break
                    }

                    if (sub === 'accept') {
                        const result = await acceptGangInvite(sender)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(
                            `🔫 Welcome to *${result.gang.name}*!\n\n` +
                            `• \`.gang deposit <amount>\` — chip into the gang vault\n` +
                            `• \`.gang info\` — see your gang's roster and vault balance\n` +
                            `• \`.gang war\` — if your gang's at war, hitting enemy members with \`.steal\`/\`.rob\` gets boosted odds\n\n` +
                            `Talk to your Leader/Officers for anything else.`,
                            [sender, result.inviterId]
                        )
                        break
                    }

                    if (sub === 'reject') {
                        const result = await rejectGangInvite(sender)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(`🚫 ${senderTag} turned down the invite to *${result.gangName}*.`, [sender, result.inviterId])
                        break
                    }

                    if (sub === 'kick') {
                        if (!mentionedJid) {
                            await reply('Tag or reply to whoever you want to kick.')
                            break
                        }
                        const result = await kickFromGang(sender, mentionedJid)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(`👋 ${targetDisplayName} was kicked from *${result.gang.name}*.`, [sender, mentionedJid])
                        break
                    }

                    if (sub === 'leave') {
                        const result = await leaveGang(sender)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(`👋 ${senderTag} left *${result.gang.name}*.`, [sender])
                        break
                    }

                    if (sub === 'promote') {
                        if (!mentionedJid) {
                            await reply('Tag or reply to whoever you want to promote.')
                            break
                        }
                        const result = await promoteInGang(sender, mentionedJid)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(`⬆️ ${targetDisplayName} is now an Officer in *${result.gang.name}*.`, [sender, mentionedJid])
                        break
                    }

                    if (sub === 'demote') {
                        if (!mentionedJid) {
                            await reply('Tag or reply to whoever you want to demote.')
                            break
                        }
                        const result = await demoteInGang(sender, mentionedJid)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(`⬇️ ${targetDisplayName} is back to a regular Member in *${result.gang.name}*.`, [sender, mentionedJid])
                        break
                    }

                    if (sub === 'transfer') {
                        if (!mentionedJid) {
                            await reply('Tag or reply to whoever should take over as leader.')
                            break
                        }
                        const result = await transferGangLeadership(sender, mentionedJid)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(`👑 ${targetDisplayName} is the new Leader of *${result.gang.name}*.`, [sender, mentionedJid])
                        break
                    }

                    if (sub === 'war') {
                        const rivalName = args.slice(1, -1).join(' ').trim()
                        const wager = parseInt(args[args.length - 1], 10)
                        if (!rivalName || isNaN(wager)) {
                            await reply('Usage: `.gang war <rival gang name> <wager amount>`\nNot sure how wars work? `.gang warhelp` has the full breakdown.')
                            break
                        }
                        const result = await declareWar(sender, rivalName, wager)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(
                            `⚔️ *${result.myGang.name}* has declared WAR on *${result.rivalGang.name}*!\n\n` +
                            `Wager: ${formatHabz(wager)} habz (locked in from ${result.myGang.name}'s vault)\n\n` +
                            `${result.rivalGang.name}'s leader has 1 hour to \`.gang waraccept\` and match the wager, or it expires and the habz goes back home.\n\n` +
                            `_\`.gang warhelp\` for the full breakdown._`
                        )
                        break
                    }

                    if (sub === 'waraccept') {
                        const result = await acceptWar(sender)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(
                            `⚔️ *${result.myGang.name}* has accepted the war against *${result.challengerGang.name}*!\n\n` +
                            `Pot: ${formatHabz(result.war.wager * 2)} habz\n\n` +
                            `For the next 24 hours, \`.steal\`/\`.rob\` against a member of the enemy gang has boosted odds. Whichever gang steals more from the other wins the whole pot. \`.gang surrender\` to bail early.`
                        )
                        break
                    }

                    if (sub === 'surrender') {
                        const result = await surrenderWar(sender)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(`🏳️ *${result.myGang.name}* has surrendered. *${result.winnerGang.name}* takes the full pot of ${formatHabz(result.pot)} habz.`)
                        break
                    }

                    if (sub === 'warhelp') {
                        await reply(
                            `⚔️ *How Gang Wars Work*\n\n` +
                            `*1. Declare*\n` +
                            `\`.gang war <rival gang name> <wager>\` — leader only. Your wager is locked out of your vault the instant you declare it, win or lose.\n\n` +
                            `*2. Accept*\n` +
                            `The rival's leader has 1 hour to \`.gang waraccept\` and match your wager from their own vault. No response in time? Challenge expires, your wager goes straight back to your vault.\n\n` +
                            `*3. Fight (24 hours)*\n` +
                            `No new commands needed. Every \`.steal\` or \`.rob\` that lands on a member of the enemy gang during the war counts as a hit and adds the amount taken to your gang's tally. Hitting an enemy gang member specifically gets boosted odds versus a normal attempt.\n\n` +
                            `*4. Bail early (optional)*\n` +
                            `\`.gang surrender\` — leader only. Instantly forfeits the full pot to the rival if you know you're losing and want to stop bleeding habz.\n\n` +
                            `*5. Resolution*\n` +
                            `After 24 hours, whichever gang's tally is higher takes the entire pot, both wagers combined, straight into their vault. A tie refunds each side their own wager. Result gets announced automatically, no command needed.\n\n` +
                            `One war at a time per gang — you can't declare a new one while already in a challenge or an active war.\n\n` +
                            `\`.gang wars\` to see your gang's win/loss record.`
                        )
                        break
                    }

                    if (sub === 'wars') {
                        const stats = await getGangWarStats(sender)
                        if (stats.error) {
                            await reply(`❌ ${stats.error}`)
                            break
                        }
                        if (!stats.wins && !stats.losses && !stats.ties) {
                            await reply(`*${stats.gang.name}* hasn't fought a war yet. \`.gang war <rival> <wager>\` to start one.`)
                            break
                        }
                        const resultEmoji = { won: '✅', lost: '❌', tie: '🤝' }
                        const recentLines = stats.recent.slice(0, 5).map((w) =>
                            `${resultEmoji[w.result]} vs *${w.opponentName}* — ${formatHabz(w.pot)} pot${w.surrendered ? ' (surrender)' : ''}`
                        )
                        await reply(
                            `⚔️ *${stats.gang.name}'s War Record*\n\n` +
                            `${stats.wins}W - ${stats.losses}L - ${stats.ties}T · ${formatHabz(stats.totalWon)} total won\n\n` +
                            `*Recent:*\n${recentLines.join('\n')}`
                        )
                        break
                    }
                    ...
                    if (sub === 'deposit') {
                        ...
                        if (isNaN(amount)) {
                            await reply('Usage: `.gang deposit <amount>`')
                            break
                        }
                        const result = await depositToGangVault(sender, amount, pushName)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(`📥 ${senderTag} deposited ${formatHabz(amount)} into *${result.gang.name}*'s vault.`, [sender])
                        break
                    }

                    if (sub === 'withdraw' || sub === 'withdrawal') {
                        const amount = parseInt(args[1], 10)
                        if (isNaN(amount)) {
                            await reply('Usage: `.gang withdraw <amount>`')
                            break
                        }
                        const result = await withdrawFromGangVault(sender, amount)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(`📤 ${senderTag} withdrew ${formatHabz(amount)} from *${result.gang.name}*'s vault.`, [sender])
                        break
                    }

                    if (sub === 'disband') {
                        const result = await disbandGang(sender)
                        if (result.error) {
                            await reply(`❌ ${result.error}`)
                            break
                        }
                        await reply(
                            result.splitAmount > 0
                                ? `💥 *${result.gang.name}* has been disbanded. Each of the ${result.members.length} members walks away with ${formatHabz(result.splitAmount)}.`
                                : `💥 *${result.gang.name}* has been disbanded.`
                        )
                        break
                    }

                    if (sub === 'info' || !sub) {
                        const info = await getGangInfo(sender)
                        if (!info) {
                            await reply("You're not in a gang. `.gang create <name>` to start one, or wait for an invite.")
                            break
                        }
                        const rankEmoji = { leader: '👑', officer: '⭐', member: '' }
                        const lines = info.members.map((m) => `${rankEmoji[m.rank]} *${m.pushName}* — ${m.rank}`)
                        await reply(
                            `🔫 *${info.name}* (${info.members.length}/10)\n\n` +
                            `Vault: ${formatHabz(info.vault_balance)}\n\n` +
                            `${lines.join('\n')}`
                        )
                        break
                    }

                    await reply('Usage: `.gang create/invite/accept/reject/kick/leave/promote/demote/transfer/deposit/withdraw/disband/info/war/waraccept/warhelp/wars/surrender`')
                    break
                }
        ...
        if (hasTrigger || isBotMentioned || isReplyToBot) {
            ...
            await sock.sendPresenceUpdate('composing', chat)
            const aiReply = await getAIReply(sender, pushName, messageContent, sender === ADMIN_JID)
            await sock.sendPresenceUpdate('paused', chat)
            // null means every provider failed too many times in a row —
            // getAIReply's own signal to go quiet instead of sending another
            // visibly-repeating fallback line. See ai.js for the full reasoning.
            if (aiReply) {
