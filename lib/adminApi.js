...
import {
    ...
    getPendingProposals, adminWipeProposal,
    getChangelogState, setChangelogState,
    getActiveEvent, startEvent, stopEvent, EVENT_TYPES,
    getWeeklyLeaderboard, getActiveGroupIds,
    getAllGangs, adminDisbandGang, adminRenameGang
} from './economy.js'
...
// Every proposal still awaiting .accept/.reject.
adminRouter.get('/proposals', async (req, res) => {
    const proposals = await getPendingProposals()
    res.json({ proposals })
})

// Wipes a single pending proposal by its row id, regardless of which side
// sent it.
adminRouter.post('/proposal/:id/wipe', async (req, res) => {
    const result = await adminWipeProposal(req.params.id)
    if (result.error) return res.status(500).json(result)
    broadcastUpdate('proposal_wiped', { id: req.params.id })
    res.json({ success: true })
})
...
// Changelog — saving a new version here is what arms the scheduler's
// next-startup broadcast (see lib/scheduler.js). Saving does NOT broadcast
// immediately by itself; it takes effect the next time the bot restarts
// after a deploy, same moment you'd already be pulling new code anyway.
adminRouter.get('/changelog', async (req, res) => {
    res.json(await getChangelogState())
})

adminRouter.post('/changelog', async (req, res) => {
    const { version, text } = req.body
    if (!version || !text) return res.status(400).json({ error: 'version and text are required' })
    const result = await setChangelogState(version, text)
    if (result.error) return res.status(500).json(result)
    res.json({ success: true })
})
...
adminRouter.get('/event', async (req, res) => {
    ...
    const config = active ? EVENT_TYPES[active.type] : null
    res.json({
        active: active ? {
            type: active.type,
            ...
            label: config.label,
            emoji: config.emoji,
            description: config.describe(active.multiplier)
        } : null
    })
})

adminRouter.post('/event/start', async (req, res) => {
    const { type, minutes, multiplier } = req.body
    const result = await startEvent(type, Number(minutes), multiplier ? Number(multiplier) : undefined, 'admin-panel')
    if (result.error) return res.status(400).json(result)

    const sock = req.app.get('sock')
    if (sock) {
        const text = `${result.config.emoji} *${result.config.label.toUpperCase()} EVENT!*\n\n${result.config.describe(result.multiplier)}\n\nEnds in ${minutes} minute${Number(minutes) === 1 ? '' : 's'}.`
        for (const groupId of await getActiveGroupIds()) {
            try { await sock.sendMessage(groupId, { text }) } catch { /* skip a group that fails, keep going */ }
        }
    }
...
adminRouter.post('/event/stop', async (req, res) => {
    const result = await stopEvent()
    if (result.error) return res.status(400).json(result)

    const sock = req.app.get('sock')
    if (sock) {
        const text = `${result.config.emoji} The *${result.config.label}* event has been ended early.`
        for (const groupId of await getActiveGroupIds()) {
            try { await sock.sendMessage(groupId, { text }) } catch { /* skip a group that fails, keep going */ }
        }
    }
...
adminRouter.get('/leaderboard/weekly', async (req, res) => {
    res.json({ leaderboard: await getWeeklyLeaderboard(10) })
})

// ---------- GANGS ----------

adminRouter.get('/gangs', async (req, res) => {
    res.json({ gangs: await getAllGangs() })
})

adminRouter.post('/gang/:id/disband', async (req, res) => {
    const result = await adminDisbandGang(req.params.id)
    if (result.error) return res.status(500).json(result)
    broadcastUpdate('gang_disbanded', { id: req.params.id })
    res.json({ success: true })
})

adminRouter.put('/gang/:id/name', async (req, res) => {
    const { name } = req.body
    const result = await adminRenameGang(req.params.id, name)
    if (result.error) return res.status(400).json(result)
    broadcastUpdate('gang_renamed', { id: req.params.id, name: result.name })
